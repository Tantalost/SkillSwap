                    ...
                    <div class="space-y-4">
                        @foreach($receivedTrades as $trade)
                            <div class="p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
                                ...
                                <p class="text-gray-700 dark:text-gray-200">
                                    Offered Card: <strong>{{ $trade->offeredCard?->card->name ?? 'Unknown Card' }}</strong>
                                </p>
                                <p class="text-gray-700 dark:text-gray-200">
                                    Requested Card: <strong>{{ $trade->requestedCard?->card->name ?? 'Unknown Card' }}</strong>
                                </p>
                                <p class="text-gray-700 dark:text-gray-200">
                                ...
                                @if($trade->status === 'pending' && $trade->receiver_id === auth()->id())
                                    <form action="{{ route('trade-requests.update', $trade->id) }}" method="POST" class="mt-2 flex gap-2">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" name="action" value="accept" class="px-3 py-1 bg-green-600 text-white rounded-md">Accept</button>
                                        <button type="submit" name="action" value="reject" class="px-3 py-1 bg-red-600 text-white rounded-md">Reject</button>
                                    </form>
                                @endif
                            </div>
                        @endforeach
                        ...
                        @foreach($sentTrades as $trade)
                            <div class="p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
                                ...
                                @if($trade->status === 'pending' && $trade->requester_id === auth()->id())
                                    <form action="{{ route('trade-requests.destroy', $trade->id) }}" method="POST" class="mt-2">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-3 py-1 bg-red-600 text-white rounded-md">Cancel</button>
                                    </form>
                                @endif
                            </div>
                        @endforeach
